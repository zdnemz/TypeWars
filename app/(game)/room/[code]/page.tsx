...
export default function RoomPage() {
  ...
  // ── send: only fires when socket is truly OPEN ──────────────
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      // Use local `ws` directly here — wsRef.current might be stale
      ws.send(
        JSON.stringify({
          type: "join_room",
          token: "",
    ...
    ws.onclose = () => {
      // Only clear the ref if it still points to THIS socket,
      // not a newer one created by a re-run of the effect
      if (wsRef.current === ws) wsRef.current = null;
    };

    return () => {
      // Detach handlers first so no stale events fire after unmount
      ws.onmessage = null;
      ws.onclose = null;

      // Send leave only if socket is already open
  ...
  const handleServerEvent = useCallback((event: ServerEvent) => {
    switch (event.type) {
      ...
      case "progress_sync":
        setRoomState((prev) =>
          prev ? { ...prev, players: event.players } : prev,
        );
        break;
      case "game_ended":
        setUIState("finished");
  ...
  if (!roomState)
    return (
      ...
      >
        <p>
          Connecting to room <strong>{code}</strong>…
        </p>
      </div>
    );
    ...
    <>
      ...
      <nav className="nav">
        <div className="nav__inner">
          <a href="/lobby" className="nav__logo">
            ⌨️ Type<span>Battle</span>
          </a>
          <div className="flex items-center gap-4">
            <div
              className="room-code"
              style={{ fontSize: "1.1rem", padding: "0.4rem 0.875rem" }}
            >
              {code}
              <button
            ...
            <span className="badge badge--accent">
              {roomState.mode === "race" ? "🏁 Race" : "🎮 Arcade"}
            </span>
          </div>
        </div>
      </nav>
      ...
      >
        ...
        {/* ── IN PROGRESS ── */}
        {(uiState === "in_progress" || uiState === "countdown") &&
          roomState.text && (
            <InProgressView
              text={roomState.text}
              mode={roomState.mode}
      ...
      {isLeader && (
        <div className="card" style={{ padding: "1.25rem" }}>
          ...
          <div className="tab-bar" style={{ maxWidth: "300px" }}>
            <button
              className={`tab-bar__tab ${mode === "race" ? "tab-bar__tab--active" : ""}`}
              onClick={() => onSetMode("race")}
            >
              🏁 Race
            </button>
            <button
              className={`tab-bar__tab ${mode === "arcade" ? "tab-bar__tab--active" : ""}`}
              onClick={() => onSetMode("arcade")}
            >
              🎮 Arcade
            </button>
          </div>
      ...
      <div className="flex gap-4">
        <div className="stat-card" style={{ flex: 1 }}>
          <div className="wpm-display">
            {wpm}
            <span className="wpm-display__unit"> wpm</span>
          </div>
          <div className="stat-card__label">Speed</div>
        </div>
        <div className="stat-card" style={{ flex: 1 }}>
          <div className="wpm-display" style={{ fontSize: "2rem" }}>
            {accuracy}
            <span className="wpm-display__unit">%</span>
          </div>
                ...
                <div key={p.id}>
                  ...
                  >
                    ...
                    <span style={{ color: "var(--text-muted)" }}>
                      {p.wpm} wpm ·{" "}
                      {Math.round(mode === "race" ? p.progress : p.score)}%
                    </span>
                  </div>
                  <div className="progress-bar">
                    <div
                      ...
                      style={{
                        width: `${mode === "race" ? p.progress : Math.min(p.score, 100)}%`,
                      }}
                    />
                  </div>
                </div>
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {results.map((r, idx) => (
          <div
            key={r.playerId}
            className={`card flex items-center gap-4 ${r.playerId === myId ? "pulse-glow" : ""}`}
            style={{
              background:
                r.playerId === myId ? "rgba(108,99,255,0.1)" : undefined,
              borderColor:
                r.playerId === myId ? "rgba(108,99,255,0.4)" : undefined,
            }}
          >

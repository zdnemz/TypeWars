...
export default function MyProfilePage() {
  ...
  useEffect(() => {
    fetch("/api/users/me")
      .then((r) => r.json())
      .then((data) => {
        setUser({ ...data, achievements: data.achievements ?? [] });
      });
  ...
  const earnedKeys = new Set(
    (user?.achievements ?? []).map((a) => a.achievement.key),
  );
      ...
      <nav className="nav">
        <div className="nav__inner">
          ...
          <div className="nav__links">
            <Link href="/lobby" className="nav__link">
              Play
            </Link>
            <button
        ...
        {user.stats && (
          <div className="grid-4" style={{ marginBottom: "2rem" }}>
            <div className="stat-card">
              <div className="stat-card__value">{user.stats.bestWpm}</div>
              <div className="stat-card__label">Best WPM</div>
            </div>
            <div className="stat-card">
              <div className="stat-card__value">
                {Math.round(user.stats.avgWpm)}
              </div>
              <div className="stat-card__label">Avg WPM</div>
            </div>
            <div className="stat-card">
              <div className="stat-card__value">{user.stats.totalGames}</div>
              <div className="stat-card__label">Games</div>
            </div>
            <div className="stat-card">
              <div className="stat-card__value">{user.stats.totalWins}</div>
        ...
        <div style={{ marginBottom: "2rem" }}>
          <h2 style={{ fontSize: "1.1rem", marginBottom: "1rem" }}>
            ...
            <span className="badge badge--accent">
              {earnedKeys.size}/{allKeys.length}
            </span>
          </h2>
          <div
            style={{
            ...
            {allKeys.map((key) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="achievement-badge__desc">
                    {earned ? data.description : "Locked"}
                  </div>
                </div>
              );
            })}
